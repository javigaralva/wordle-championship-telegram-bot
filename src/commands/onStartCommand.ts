...
export async function onStartCommandHandler( msg: TelegramBot.Message ) {
    const text =
        `*Hola ${msg.from?.first_name ?? 'personaje misterioso'} 👋 ¡Bienvenido a Wordle Tildes Championship!*\n` +
        //`*Hola ${msg.from?.first_name ?? 'personaje misterioso'} 👋 ¡Bienvenido a Wordle Tildes Championship!*\n` +
        '🏁 Cada lunes comienza automáticamente un nuevo campeonato.\n' +
        '📨 Para participar solo me tienes que reenviar el resultado desde la web de https://wordle.danielfrg.com (versión *TILDES*) cuando termines la partida.\n'
